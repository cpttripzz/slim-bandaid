...
$app->post('/signin', function () use ($app) {
    ...
    if ($_POST['username'] === 'alex' && $_POST['password'] === 'password') {
        // Add the user ID to the auth params and forward the user to authorize the client
        $params['user_id'] = 1;
        // $params['user_id'] = $_SESSION['user_id'];
        $_SESSION['params'] = serialize($params);
        $app->redirect('/oauth2-example-auth-server/oauth.php/authorize');
...
$app->get('/authorize', function () use ($app) {
    ...
    <p>
        The application <strong><?php echo $params['client_details']['name']; ?></strong> would like permission to
        access your:
    </p>
...
$app->post('/authorize', function () use ($server, $app) {
    ...
    if (isset($_POST['approve']) || $autoApprove === true) {
        ...
        echo '<p>The user authorized a request and so would be redirected back to the client...</p>';
